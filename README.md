...
Репозиторій публічний. Не додавайте сюди паролі, токени, приватні ключі, персональні дані або логи. `.gitignore` і `build.py` навмисно зберігаються в репозиторії, тому вони видимі всім. Каталоги `logs/`, `release/` і локальне середовище Python ігноруються.
